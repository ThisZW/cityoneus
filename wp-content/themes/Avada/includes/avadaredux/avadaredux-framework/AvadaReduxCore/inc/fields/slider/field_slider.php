...
if ( ! class_exists( 'AvadaReduxFramework_slider' ) ) {
	class AvadaReduxFramework_slider {
		 ...
		 *
		 * @since AvadaRedux_Framework 3.1.8
		 */
		function clean() {
		...
		function enqueue() {
			...
			wp_enqueue_style(
				...
				AvadaReduxFramework::$_url . 'inc/fields/slider/vendor/nouislider/avadaredux.jquery.nouislider.css',
				array(),
				'5.0.0',
				'all'
			);
			...
			wp_register_script(
				...
				AvadaReduxFramework::$_url . 'inc/fields/slider/vendor/nouislider/avadaredux.jquery.nouislider' . $min . '.js',
				array( 'jquery' ),
				'5.0.0',
				true
			);
			...
			wp_enqueue_script(
				'avadaredux-field-slider-js',
				AvadaReduxFramework::$_url . 'inc/fields/slider/field_slider' . $min . '.js',
				array( 'jquery', 'avadaredux-nouislider-js', 'avadaredux-js', 'select2-js' ),
				time(),
				true
			);
			...
			if ($this->parent->args['dev_mode']) {
				wp_enqueue_style(
					'avadaredux-field-slider-css',
					AvadaReduxFramework::$_url . 'inc/fields/slider/field_slider.css',
					array(),
					time(),
					'all'
				);
		...
		function render() {
			...
			if ( $this->display_text == $this->field['display_value'] ) {
				...
				echo '<input type="text"
						 ...
						 id="' . $idOne . '"
						 value="' . $valOne . '"
						 class="avadaredux-slider-input avadaredux-slider-input-one-' . $fieldID . ' ' . $this->field['class'] . '"/>';
			...
			// LABEL output
				...
				echo '<div class="avadaredux-slider-label' . $labelNum . '"
					   id="avadaredux-slider-label-one-' . $fieldID . '"
					   name="' . $nameOne . '">
				  </div>';
			...
			// SELECT output
				...
				echo '<select class="avadaredux-slider-select-one avadaredux-slider-select-one-' . $fieldID . ' ' . $this->field['class'] . '"
						  name="' . $nameOne . '"
						  id="' . $idOne . '">
				 </select>';
			}
			...
			echo
			'<div
				class="avadaredux-slider-container ' . $this->field['class'] . '"
				id="' . $fieldID . '"
				data-id="' . $fieldID . '"
			...
			if ( true == $twoHandles ) {
				...
				if ( true == $showInput ) {
					echo '<input type="text"
							 ...
							 id="' . $idTwo . '"
							 value="' . $valTwo . '"
							 class="avadaredux-slider-input avadaredux-slider-input-two-' . $fieldID . ' ' . $this->field['class'] . '"/>';
				}

				// LABEL
				if ( true == $showLabel ) {
					echo '<div class="avadaredux-slider-label-two"
						   id="avadaredux-slider-label-two-' . $fieldID . '"
						   name="' . $nameTwo . '">
					  </div>';
				}
				...
				// SELECT
				if ( true == $showSelect ) {
					echo '<select class="avadaredux-slider-select-two avadaredux-slider-select-two-' . $fieldID . ' ' . $this->field['class'] . '"
							  name="' . $nameTwo . '"
							  id="' . $idTwo . '">
					 </select>';
			...
			if ( $this->display_none == $this->field['display_value'] || $this->display_label == $this->field['display_value'] ) {
				echo '<input type="hidden"
						 class="avadaredux-slider-value-one-' . $fieldID . ' ' . $this->field['class'] . '"
						 name="' . $nameOne . '"
						 id="' . $idOne . '"
						 value="' . $valOne . '"/>';
				...
				if ( true == $twoHandles ) {
					echo '<input type="hidden"
							 class="avadaredux-slider-value-two-' . $fieldID . ' ' . $this->field['class'] . '"
							 name="' . $nameTwo . '"
							 id="' . $idTwo . '"
							 value="' . $valTwo . '"/>';
				}
